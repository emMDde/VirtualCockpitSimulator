...
ControllerManager::ControllerManager(QObject *parent) : QObject(parent), _serial(new QSerialPort(this)), _reconnectTimer(new QTimer(this))
{
...
void ControllerManager::generateTestData()
{
    ...
    float rotX = 180.0f * std::sin(_testStep);
...
bool ControllerManager::searchForController()
{
    for (const QSerialPortInfo &port : QSerialPortInfo::availablePorts()) {
        ...
        {
            _serial->write("WDS_KONTROLLER_CONNECT");

            if (_serial->waitForReadyRead(200))
            {
                QByteArray response = _serial->readAll();
                if (response.contains("WDS_KONTROLLER_CONNECTED"))
                {
                    qDebug() << "Znaleziono urządzenie na porcie:" << port.portName();
...
bool ControllerManager::checkCRC(const QByteArray &data)
{
    return true;
}
...
void ControllerManager::readData()
{
    ...
    while (_buffer.size() >= PACKET_SIZE)
    {
        ...
        std::memcpy(&rotZ, _buffer.constData() + 16, sizeof(float));
        std::memcpy(&crc, _buffer.constData() + 20, sizeof(crc));
